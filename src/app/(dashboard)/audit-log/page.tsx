  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <header className="mb-6">
        <h1 className="text-3xl font-bold text-gray-600">Audit Log</h1>
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-4">
          <div className="xl:col-span-2">
            <label htmlFor="searchQuery" className="text-sm font-medium text-gray-700 block mb-1">Search</label>
            <div className="relative">
              <FiSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
              <input
                type="text" name="searchQuery" id="searchQuery"
          ...
          <div>
            <label htmlFor="actionType" className="text-sm font-medium text-gray-700 block mb-1">Action Type</label>
            <select name="actionType" id="actionType" value={filters.actionType} onChange={handleFilterChange} className="w-full py-2 px-3 border border-gray-300 rounded-md">
          ...
          <div>
            ...
            <select name="status" id="status" value={filters.status} onChange={handleFilterChange} className="w-full py-2 px-3 border border-gray-300 rounded-md">
              <option value="">All Statuses</option>
              {statusOptions.map((s) => (<option key={s} value={s}>{s}</option>))}
            </select>
          </div>
          <div className="flex items-end">
            <button onClick={clearFilters} className="w-full py-2 px-4 text-sm font-medium text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200">
              Clear
            </button>
          </div>
        </div>
      ...
      <div className="overflow-x-auto bg-white rounded-lg shadow-sm border border-gray-200">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {isLoading && (
              <tr><td colSpan={6} className="py-10"><div className="flex justify-center">Loading...</div></td></tr>
            )}
            {error && (
              <tr><td colSpan={6} className="text-center py-10 text-red-500">Failed to load data.</td></tr>
            )}
            ...
            {data?.logs?.map((log) => (
              <tr key={log.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800 font-medium">{log.userFullName}</td>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm">
                  {log.status === "SUCCESS" ? (
                    <span className="flex items-center gap-2 text-green-700"><FiCheckCircle /> Success</span>
                  ) : (
                    <span className="flex items-center gap-2 text-red-700"><FiAlertCircle /> Failure</span>
                  )}
                </td>
              </tr>
            ))}
      ...
      <div className="flex justify-between items-center mt-4">
        ...
        <div className="flex gap-2">
          <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1 || isLoading} className="py-1 px-3 border rounded-md bg-white disabled:opacity-50">
            Previous
          </button>
          <button onClick={() => setPage((p) => p + 1)} disabled={page >= (data?.totalPages || 1) || isLoading} className="py-1 px-3 border rounded-md bg-white disabled:opacity-50">
            Next
          </button>
        </div>
      </div>
